% Input:
%--------
% Data "PC3_14_Split_1_2A_Split" -> Data split 2A
% Data "PC3_16_DataFitur_2A"  -> Data training untuk 2A

% ----------------------------------------------------------------------------------------------------------
% Cari jumlah TRUE dan FALSE serta nilai ENTROPY children di Mtraining berdasarkan "PC3_14_Split_1_2A_Split"
% ----------------------------------------------------------------------------------------------------------
    
for iKolomCellA = 1 : 37 % Iterasi fitur PC3 ada 37 (exclude kelas)       
    %--      
    panjangSplit2A = length(PC3_14_Split_1_2A_Split{1,iKolomCellA}); % Setiap DATA SPLIT diulang sebanyak jumlah DATA TRAINING
    panjangTraining2A = size(PC3_16_DataFitur_2A{1,iKolomCellA},1); % Iterasi data training AGAR MATCH dengan SATU DATA split    
    for iBarisSplitA = 1 : panjangSplit2A  % data SPLIT 2A       
        ...
        for iBarisTrainingA = 1 : panjangTraining2A % data TRAINING 2A            
            ...
            % -----------------------------------------------------------                            
            dataTrainingA = PC3_16_DataFitur_2A{1, iKolomCellA}(iBarisTrainingA,1); % Data training
            dataKelasA = PC3_16_DataFitur_2A{1, iKolomCellA}(iBarisTrainingA,2); % Data kelas                          
            dataSplitA = PC3_14_Split_1_2A_Split{1, iKolomCellA}(iBarisSplitA,1); % Data split                                        
        ...
        % ---------------------------
        PC3_14_Split_1_2A_Split{1,iKolomCellA}(iBarisSplitA,2) = jmlTrueKurangA; % Jumlah TRUE dengan parameter ( <= ) disimpan di kolom 2
        PC3_14_Split_1_2A_Split{1,iKolomCellA}(iBarisSplitA,3) = jmlFalseKurangA; % Jumlah FALSE dengan parameter ( <= ) disimpan di kolom 3
        PC3_14_Split_1_2A_Split{1,iKolomCellA}(iBarisSplitA,5) = jmlTrueLebihA; % Jumlah TRUE dengan parameter ( > ) disimpan di kolom 5
        PC3_14_Split_1_2A_Split{1,iKolomCellA}(iBarisSplitA,6) = jmlFalseLebihA; % Jumlah FALSE dengan parameter ( > ) disimpan di kolom 6                                
        ...
        PC3_14_Split_1_2A_Split{1,iKolomCellA}(iBarisSplitA,4) = entropyChildKurangA(iBarisSplitA,1); % Nilai entropy child dari parameter ( <= ) disimpan di kolom 4                          
        ...
        PC3_14_Split_1_2A_Split{1,iKolomCellA}(iBarisSplitA,7) = entropyChildLebihA(iBarisSplitA,1); % Nilai entropy child dari parameter ( > ) disimpan di kolom 7 

        % -----------------------------------------
        % Mencari nilai INFO dari setiap data split
        % -----------------------------------------
        dataChildKurangA = (totalKurangA/jmlData) * PC3_14_Split_1_2A_Split{1, iKolomCellA}(iBarisSplitA,4);
        dataChildLebihA = (totalLebihA/jmlData) * PC3_14_Split_1_2A_Split{1, iKolomCellA}(iBarisSplitA,7);
        INFOsplitA(iBarisSplitA,1) = (dataChildKurangA + dataChildLebihA);
        PC3_14_Split_1_2A_Split{1,iKolomCellA}(iBarisSplitA,8) = INFOsplitA(iBarisSplitA,1); % nilai INFO dari data SPLIT. disimpan di kolom 8

        % ------------------------------------
        % Mencari nilai GAIN dari setiap INFO
        % ------------------------------------
        GAINinfoA(iBarisSplitA,1) = PC3_03_Keterangan(1,4) - INFOsplitA(iBarisSplitA,1);
        PC3_14_Split_1_2A_Split{1,iKolomCellA}(iBarisSplitA,9) = GAINinfoA(iBarisSplitA,1); % nilai INFO dari data SPLIT. disimpan di kolom 9                        

        % ----------------------------------------------------------------------------------------------------------------------------
        % Penyederhanaan variable "PC3_14_Split_1_2A_Split" 
    ...
    % ---------------------------------------------------------------
    [NilaiA,BarisKeA] = max(PC3_14_Split_1_2A_Split{1,iKolomCellA}(:,9)); % Ambil urutan ke berapa si split terbaik itu dan ambil nilai max gain-nya
    angkaSplitA = PC3_14_Split_1_2A_Split{1, iKolomCellA}(BarisKeA,1); % Angka split terbaik dari daftar urut split
    PC3_18_BEST_Split_2A{1,iKolomCellA} = [BarisKeA angkaSplitA NilaiA]; % nilai max Gain dari data split ke berapa                
%--
end        
...
% "PC3_14_Split_1_2A_Split" dengan total 9 kolom
% Nilai Gain (MAX) sebagai split terbaik "PC3_18_BEST_Split_2A"